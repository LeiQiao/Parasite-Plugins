...
def gen_token():
    # 生成 token
    token = SESSION_PREFIX + str(uuid.uuid1()).replace('-', '')
    return custom_success({'token': token})
...
def gen_captcha():
    ...
    if token is None:
        raise parameter_error(i18n(TOKEN_EXPIRED))
    ...
    redis.set_json(token, user_session, ldap_config.unlogin_session_expire)
    ...
    resp.set_cookie('token', token, expires=expire_timestamp)
...
def login():
    ...
    if captcha is None or len(captcha) == 0 or \
       token is None or len(token) == 0 or not token.startswith(SESSION_PREFIX):
    ...
    redis_client.set_json(token, user_session, ldap_config.logined_session_expire)
